...
export default function SimulatorLayout(): ReactElement {
  ...
  return (
    <ThemeProvider>
      <div className="flex h-[100dvh] theme-bg-main theme-text-main font-sans overflow-hidden relative selection:bg-blue-100 flex-col md:flex-row transition-colors duration-500">
        <ThemeSwitcher />
        ...
        <header className="md:hidden bg-white border-b border-gray-200 p-4 flex justify-between items-center z-40 shrink-0 shadow-sm theme-bg-card theme-border">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-gray-900 rounded-lg flex items-center justify-center text-white shadow-sm theme-btn-primary">
              <LayoutDashboard size={18} />
            </div>
            <span className="font-bold text-gray-900 tracking-tight theme-text-main">Imob-Invest</span>
          </div>
          <button
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            className="p-2 text-gray-500 hover:bg-gray-100 rounded-md transition-colors theme-text-muted"
            aria-label="Menu"
          >
        ...
        <aside
          className={`
            fixed md:relative top-0 left-0 h-[100dvh] md:h-full w-[280px] md:w-80 theme-bg-card border-r theme-border flex flex-col z-50 transform transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-2xl md:shadow-none
            ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
        `}
        >
          <div className="p-6 border-b theme-border theme-bg-card">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 theme-btn-primary rounded-lg flex items-center justify-center text-white shadow-sm">
                <LayoutDashboard size={18} />
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar theme-bg-card">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div
                className={`w-full flex items-center justify-between p-2.5 rounded-lg transition-all font-medium text-sm group ${viewMode === 'EDITOR' && editorTab === 'FINANCING' ? 'bg-gray-100 theme-text-main' : 'bg-transparent theme-text-muted hover:bg-gray-50'}`}
              >
                <button
                  onClick={() => {
                    setViewMode('EDITOR')
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 p-2.5 rounded-lg transition-all font-medium text-sm ${viewMode === 'EDITOR' && editorTab === 'RENTAL' ? 'bg-rose-50 text-rose-700' : 'bg-transparent theme-text-muted hover:bg-gray-50'}`}
              >
            ...
            <div className="mt-2 space-y-1">
              <Button
                ...
                variant="ghost"
                fullWidth
                className={`justify-start gap-3 theme-text-muted ${viewMode === 'GLOBAL_SETTINGS' ? 'bg-gray-100 theme-text-main font-bold' : ''}`}
              >
        ...
        <main className="flex-1 overflow-hidden relative w-full theme-bg-main">
