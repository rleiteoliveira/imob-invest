                ...
                <div className="animate-in fade-in slide-in-from-right-2">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <NumberInput
                      ...
                      onChange={(val: any) => setData({ ...data, constructionTime: val })}
                    />
                    <span className="text-sm font-bold text-orange-800">Meses</span>
                  </div>
                  <div><label className="text-[10px] font-bold text-orange-700 block mb-1 uppercase tracking-wide">Executado (%)</label><NumberInput className="w-full p-2.5 border border-orange-200 rounded-lg bg-white shadow-sm font-medium" value={data.currentWorkPercent} onChange={(val: any) => setData({ ...data, currentWorkPercent: val })} /></div>
                </div>
              )}

              <div className="grid grid-cols-1 gap-3 pt-2">
                <div><label className="text-[10px] font-bold text-orange-700 block mb-1 uppercase tracking-wide">INCC (% a.m)</label><NumberInput allowFloat={true} className="w-full p-2.5 border border-orange-200 rounded-lg bg-white shadow-sm font-medium" value={data.inccRate} onChange={(val: any) => setData({ ...data, inccRate: val })} /></div>
              </div>
                        ...
                        <div>
                          {data.balloonFrequency === 'UNICA' ? (
                            <>
                              ...
                              <NumberInput placeholder="0" className="w-full p-2.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-500 font-medium shadow-sm" value={data.balloonStartMonth ?? ''} onChange={(val: any) => setData({ ...data, balloonStartMonth: val })} />
                            </>
                          ) : (
                            <>
                              <label className="text-[10px] text-gray-500 uppercase font-bold mb-1 block tracking-wide">Qtd. Vezes</label>
                              <NumberInput className="w-full p-2.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-500 font-medium shadow-sm" value={data.balloonCount} onChange={(val: any) => setData({ ...data, balloonCount: val })} />
                            </>
                          )}
                        </div>
                      </div>
